...
Function PontoParaVirgula(valor)
        PontoParaVirgula = Replace(valor, ".", ",")
End Function   

Function VirgulaParaPonto(valor)
        VirgulaParaPonto = Replace(valor, ",", ".")
End Function 
...
Function LimparValorString(valorString)
    ' Função específica para limpar valores vindos do banco de dados
    On Error Resume Next
    
    Dim valorLimpo
    valorLimpo = ""
    
    If Not IsNull(valorString) Then
        valorLimpo = CStr(valorString)
        ' Remove pontos (separadores de milhar)
        valorLimpo = Replace(valorLimpo, ".", "")
        ' Substitui vírgula (separador decimal) por ponto
        valorLimpo = Replace(valorLimpo, ",", ".")
        ' Remove espaços e símbolos
        valorLimpo = Replace(valorLimpo, "R$", "")
        valorLimpo = Replace(valorLimpo, "$", "")
...
Function IsNumericValue(valor)
    ...
    If Not IsNull(valor) Then
        ' Primeiro limpa o valor
        Dim valorTemp
        valorTemp = LimparValorString(valor)
...
If Not rsDadosGerais.EOF Then
    ...
    ' Tratamento seguro para totalVGV - USANDO A NOVA FUNÇÃO
    If Not IsNull(rsDadosGerais("TotalVGV")) Then
        Dim valorVGVString
        valorVGVString = CStr(rsDadosGerais("TotalVGV"))
        totalVGV = ConverterValor(valorVGVString)
    End If
    
    totalVGV = rsDadosGerais("TotalVGV")
    ' Tratamento seguro para totalComissao - USANDO A NOVA FUNÇÃO
    If Not IsNull(rsDadosGerais("TotalComissao")) Then
        Dim valorComissaoString

        totalComissao = rsDadosGerais("TotalComissao")
...
Do While Not rsMensal.EOF
    ...
    ' Tratamento seguro para totalVGVMes - COM LIMPEZA DOS VALORES
    totalVGVMes = 0
    If Not IsNull(rsMensal("TotalVGV")) Then
        Dim valorVGVMesString
        valorVGVMesString = CStr(rsMensal("TotalVGV"))
        totalVGVMes = ConverterValor(valorVGVMesString)
    End If
    
    ' Tratamento seguro para totalComissaoMes - COM LIMPEZA DOS VALORES
    totalComissaoMes = 0
    If Not IsNull(rsMensal("TotalComissao")) Then
        Dim valorComissaoMesString
        valorComissaoMesString = CStr(rsMensal("TotalComissao"))
        ' Remove pontos e substitui vírgula por ponto
        totalComissaoMes = ConverterValor(valorComissaoMesString)
        totalComissaoMes = valorComissaoMesString
    End If
...
Do While Not rsEmpreendimentos.EOF
    ...
    ' VGV Total - COM LIMPEZA DOS VALORES
    vgvEmp = 0
    If Not IsNull(rsEmpreendimentos("TotalVGV")) Then
        Dim valorVGVEmpString
        valorVGVEmpString = CStr(rsEmpreendimentos("TotalVGV"))
        ' Remove pontos e substitui vírgula por ponto
        valorVGVEmpString = Replace(valorVGVEmpString, ".", "")
        valorVGVEmpString = Replace(valorVGVEmpString, ",", ".")
        vgvEmp = ConverterValor(valorVGVEmpString)
    End If
...
Do While Not rsLocalidades.EOF
    ...
    If Not IsNull(rsLocalidades("TotalVGV")) Then
        Dim valorVGVLocalString
        valorVGVLocalString = CStr(rsLocalidades("TotalVGV"))
        ' Remove pontos e substitui vírgula por ponto
        valorVGVLocalString = Replace(valorVGVLocalString, ".", "")
        valorVGVLocalString = Replace(valorVGVLocalString, ",", ".")
        vgvLocal = ConverterValor(valorVGVLocalString)
    End If
...
Do While Not rsTopVendas.EOF
    ...
    ' Valor - COM LIMPEZA DOS VALORES
    valorVenda = 0
    If Not IsNull(rsTopVendas("ValorUnidade")) Then
        Dim valorUnidadeString
        valorUnidadeString = CStr(rsTopVendas("ValorUnidade"))
        ' Remove pontos e substitui vírgula por ponto

        valorVenda = ConverterValor(valorUnidadeString)
    End If
    
    ' Valor formatado
    valorVenda = PontoParaVirgula(valorUnidadeString)
    valorFormatado = "R$ " & FormatNumber(PontoParaVirgula(valorVenda), 2)
