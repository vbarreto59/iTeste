...
If (request.ServerVariables("remote_addr") <> "127.0.0.1") AND (request.ServerVariables("remote_addr") <> "::1") Then
    ...
    ' =================== BUSCA TOTAIS DE VENDAS POR MÊS E ANO ===================
    Dim sqlTotais, rsTotais, anoAtual, mesAtual
    Dim totalMes, totalAno, corpoEmail
    
    ' Pega ano e mês atual para filtro
    anoAtual = Year(Date())
    mesAtual = Month(Date())
    
    ' Busca total do mês atual
    sqlTotais = "SELECT SUM(ValorUnidade) AS TotalMes FROM Vendas " & _
                "WHERE Excluido = 0 AND AnoVenda = " & anoAtual & " " & _
                "AND MesVenda = " & mesAtual
    
    Set rsTotais = connSales.Execute(sqlTotais)
    If Not rsTotais.EOF And Not IsNull(rsTotais("TotalMes")) Then
        totalMes = FormatNumber(rsTotais("TotalMes"), 2)
    Else
        totalMes = "0,00"
    End If
    rsTotais.Close
    
    ' Busca total do ano atual
    sqlTotais = "SELECT SUM(ValorUnidade) AS TotalAno FROM Vendas " & _
                "WHERE Excluido = 0 AND AnoVenda = " & anoAtual
    
    Set rsTotais = connSales.Execute(sqlTotais)
    If Not rsTotais.EOF And Not IsNull(rsTotais("TotalAno")) Then
        totalAno = FormatNumber(rsTotais("TotalAno"), 2)
    Else
        totalAno = "0,00"
    End If
    rsTotais.Close
    
    ' Busca totais por mês do ano atual
    sqlTotais = "SELECT MesVenda, SUM(ValorUnidade) AS Total FROM Vendas " & _
                "WHERE Excluido = 0 AND AnoVenda = " & anoAtual & " " & _
                "GROUP BY MesVenda ORDER BY MesVenda"
    
    Set rsTotais = connSales.Execute(sqlTotais)
    
    ' Monta string com totais por mês
    Dim totaisPorMes, mesNome
    totaisPorMes = ""
    
    If Not rsTotais.EOF Then
        totaisPorMes = "TOTAIS POR MÊS - ANO " & anoAtual & ":" & vbCrLf & vbCrLf
        Do While Not rsTotais.EOF
            Dim mesNum, mesTotal
            mesNum = rsTotais("MesVenda")
            ...
            Select Case mesNum
                ...
                Case Else: mesNome = "Mês " & mesNum
            End Select
            
            If Not IsNull(rsTotais("Total")) Then
                mesTotal = FormatNumber(rsTotais("Total"), 2)
            Else
                mesTotal = "0,00"
            End If
            
            totaisPorMes = totaisPorMes & mesNome & ": R$ " & mesTotal & vbCrLf
            rsTotais.MoveNext
        Loop
    Else
        totaisPorMes = "Nenhuma venda encontrada para o ano " & anoAtual
    End If
    rsTotais.Close
    Set rsTotais = Nothing
    ' ===========================================================================
    
    ' Monta o corpo do email com todas as informações
    corpoEmail =              "===========================================" & vbCrLf
    corpoEmail = corpoEmail & "NOVA VENDA INSERIDA COM SUCESSO" & vbCrLf
    corpoEmail = corpoEmail & "===========================================" & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & "ID DO REGISTRO: " & vendaId & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & "SQL EXECUTADO: " & sqlVendas & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & "--------------------------------" & vbCrLf
    corpoEmail = corpoEmail & "RESUMO FINANCEIRO" & vbCrLf
    corpoEmail = corpoEmail & "--------------------------------" & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & "TOTAL DO MÊS ATUAL (" & mesNome & "): R$ " & totalMes & vbCrLf
    corpoEmail = corpoEmail & "TOTAL DO ANO ATUAL (" & anoAtual & "): R$ " & totalAno & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & totaisPorMes & vbCrLf & vbCrLf
    corpoEmail = corpoEmail & "===========================================" & vbCrLf
    corpoEmail = corpoEmail & "Data/Hora: " & Now() & vbCrLf
    corpoEmail = corpoEmail & "Usuário: " & Ucase(Session("Usuario")) & vbCrLf
