    ...
    For k = 1 To qtdPorMes
        ...
        If Err.Number = 0 Then
            FlushLine "[" & Right("0"&diaVenda,2) & "/" & Right("0"&mesVenda,2) & "/" & anoVenda & "] " & _
                      ...
                      " | Gerência: " & Server.HTMLEncode(nomeGer) & _
                      " | Corretor: " & Server.HTMLEncode(nomeCor) & _
            ...
            sqlVerify = "SELECT TOP 1 NomeEmpreendimento, UserIdDiretoria, NomeDiretor, UserIdGerencia, NomeGerente FROM Vendas WHERE Empreend_ID = " & idEmp & " AND DataVenda = " & SqlDate(dataVenda)
            Set rsVerify = connSales.Execute(sqlVerify)
            If Not rsVerify.EOF Then
                FlushLine "Verificação: NomeEmpreendimento = " & Server.HTMLEncode(rsVerify("NomeEmpreendimento")) & _
                          ", UserIdDiretoria = " & rsVerify("UserIdDiretoria") & _
                          ", NomeDiretor = " & Server.HTMLEncode(rsVerify("NomeDiretor")) & _
                          ", UserIdGerencia = " & rsVerify("UserIdGerencia") & _
                          ", NomeGerente = " & Server.HTMLEncode(rsVerify("NomeGerente"))
            Else
...
If Err.Number = 0 Then
    ...
    If Not rsVerifyDir.EOF Then
        FlushLine "Verificação Diretoria: UserIdDiretoria = " & rsVerifyDir("UserIdDiretoria") & ", NomeDiretor = " & Server.HTMLEncode(rsVerifyDir("NomeDiretor"))
    Else
...
If Err.Number = 0 Then
    ...
    If Not rsVerifyGer.EOF Then
        FlushLine "Verificação Gerência: UserIdGerencia = " & rsVerifyGer("UserIdGerencia") & ", NomeGerente = " & Server.HTMLEncode(rsVerifyGer("NomeGerente"))
    Else
