...
Function RemoverNumeros(texto)
    ...
    ' Pattern to match numbers (0-9) and asterisks (*)
    regex.Pattern = "[0-9*-]"
    regex.Global = True
    ...
    ' Remove extra spaces (optional: replaces double spaces with single, then trims)
    RemoverNumeros = Trim(Replace(RemoverNumerosEAsteriscos, "  ", " "))
End Function

' Retrieve success message from query string, if any
Dim mensagem
...
'-------------------------------------------------------------------------------'
' Cria explicitamente o objeto Connection para o banco de vendas
Set connSales = Server.CreateObject("ADODB.Connection")

' Abre a conexão com a string específica para o banco de vendas
connSales.Open StrConnSales

' Cria e abre um ADODB Recordset
Set rs = Server.CreateObject("ADODB.Recordset")

' Query modificada para usar JOIN entre bancos diferentes
sql = "SELECT Vendas.*, Usuarios.Nome AS UsuarioNome " & _
      "FROM Vendas " & _
      "LEFT JOIN [;DATABASE=" & dbSunnyPath & "].Usuarios ON Vendas.CorretorId = Usuarios.UserId " & _
      "WHERE (Vendas.Excluido <> -1 OR Vendas.Excluido IS NULL) " & _
...
' Executa usando a conexão do banco de vendas
...
If rs.EOF And rs.BOF Then
 '' Response.Write "A consulta não retornou resultados. Verifique se há dados no banco de dados e se a consulta está correta."
Else
''  Response.Write "A consulta retornou " & rs.RecordCount & " registros."
End If

'-------------------------------------------------------------'
        
' Inicia o bloco de JavaScript para popular o array salesData
Response.Write "<script>"
Response.Write "var salesData = [];"
'Dim totalValorScript, totalComissaoScript
totalValorScript = 0
totalComissaoScript = 0

If Not rs.EOF Then
    ' Loop através dos registros para popular o array JavaScript
    Do While Not rs.EOF
        Dim comissaoPercentual, valorComissaoGeral
        ' Formata os valores de comissão para exibição
        comissaoPercentual = FormatNumber(rs("ComissaoPercentual"), 2)
        valorComissaoGeral = FormatNumber(rs("ValorComissaoGeral"), 2)
        
        ' Acumula os totais para o lado do JavaScript (para cartões mobile)
        ...
        ' Adiciona os dados de vendas ao array JavaScript
    ...
    Loop
End If
' Move o cursor do recordset de volta para o primeiro registro para popular a tabela HTML
rs.MoveFirst
'rs.Close

' A segunda abertura do recordset não é necessária, pois os dados já foram capturados para o JavaScript.
' Se a tabela HTML for populada no servidor, a lógica de JOIN precisaria ser replicada aqui novamente.
' SQL query to retrieve sales data and user name
' sql= "SELECT Vendas.*, Usuarios.Nome AS UsuarioNome FROM Vendas INNER JOIN Usuarios ON Vendas.CorretorId = Usuarios.UserId WHERE (Vendas.Excluido <> -1 OR Vendas.Excluido IS NULL) ORDER BY Vendas.ID DESC;"
' rs.Open sql, conn
    ...
    <style>
       ...
       .card-body {
        ...
        @media (max-width: 767.98px) {
            ...
            /* Cards */
            #tabelaVendas, .dataTables_wrapper {
...
<body>
    <div class="container-fluid">
        ...
        <div class="mb-4 d-grid gap-2 d-md-block">
            ...
            <a href="inserirVendasTeste2.asp" class="btn btn-success" target="_blank"><i class="fas fa-plus"></i> Inserir Testes</a>            
        </div>
...
If Not rs.EOF Then
    ...
    Do While Not rs.EOF
        ' ------------------------------------------------------------
        ' Lógica de verificação de pagamento e construção do tooltip (NOVA)
        ' ------------------------------------------------------------
        Dim sqlPagamentos, rsPagamentos
        Dim totalPagoDiretoria, totalPagoGerencia, totalPagoCorretor
        Dim dataPagamentoDiretoria, dataPagamentoGerencia, dataPagamentoCorretor ' Novas variáveis para a data
        Dim tooltipDiretoria, tooltipGerencia, tooltipCorretor
        Dim pagoDiretoria, pagoGerencia, pagoCorretor
                
        ' Inicializa as variáveis para cada venda
        ...
        ' Consulta todos os pagamentos para a venda atual
        ...
        ' Loop para processar e formatar os pagamentos por tipo
        If Not rsPagamentos.EOF Then
            Do While Not rsPagamentos.EOF
                ' Monta a linha de detalhe do pagamento
                ...
                ' Adiciona ao tooltip e ao total pago conforme o tipo de recebedor
                Select Case LCase(rsPagamentos("TipoRecebedor"))
                    Case "diretoria"
                        ...
                        ' Armazena a última data encontrada
        ...
        ' Define o status de PAGO comparando os totais
        If rs("ValorDiretoria") > 0 And totalPagoDiretoria >= CDbl(rs("ValorDiretoria")) Then pagoDiretoria = True
        If rs("ValorDiretoria") = 0 Then pagoDiretoria = True ' Caso não haja comissão, já é considerado pago
        ...
        <tr>
            ...
            <td>
                ...
                <% If Not IsEmpty(dataPagamentoDiretoria) Then %>
                    <br><small style="color: blue;">Pag.: <%= dataPagamentoDiretoria %></small>
                <% End If %>
            ...
            <td>
                ...
                <br><small style="color: blue;">Acum.: <%= FormatNumber(rs("ValorDiretoria")+rs("ValorGerencia"), 2) %></small>
                <% If Not IsEmpty(dataPagamentoGerencia) Then %>
                    <br><small style="color: blue;">Pag.: <%= dataPagamentoGerencia %></small>
                <% End If %>
            ...
            <td>
                ...
                <br><small style="color: blue;">Acum.: <%= FormatNumber(rs("ValorCorretor")+rs("ValorDiretoria")+rs("ValorGerencia"), 2) %></small>
                <% If Not IsEmpty(dataPagamentoCorretor) Then %>
                    <br><small style="color: blue;">Pag.: <%= dataPagamentoCorretor %></small>
                <% End If %>
...
End If
    ...
    $(document).ready(function () {
        var table; // DataTable global
        var originalSalesData = []; // Armazena todos os dados originais para pesquisa
        ...
        // Função de pesquisa para mobile
...
<div class="sale-card">
    ...
    <div class="card-actions">
        ...
        // Detecta tamanho da tela e adapta interface
        function checkScreenSize() {
            if (window.matchMedia('(max-width: 767.98px)').matches) {
                ...
                // Armazena os dados originais para pesquisa
                originalSalesData = salesData;
                renderMobileCards(originalSalesData);
                
                // Configura evento de pesquisa para mobile
