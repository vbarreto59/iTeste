...
<body>
    <div class="container-fluid mt-5">
        ...
        <a href="gestao_vendas_comissao_saldo1.asp" class="btn btn-success" target="_blank"><i class="fas fa-plus"></i> Saldos</a><br><br>
                ...
                <tbody>
                    ...
                    If Not rsComissoes.EOF Then
                        Do While Not rsComissoes.EOF
                            ...
                            sqlPagamentos = "SELECT SUM(ValorPago) as ValorTotalPago, MAX(DataPagamento) as DataPagamento FROM PAGAMENTOS_COMISSOES " & _
                                            "WHERE ID_Venda = " & vendaId & " AND UserId = " & userIdDiretoria & " AND TipoRecebedor = 'diretoria'"
                            ...
                            ' #### Pagamentos para Gerência
                            sqlPagamentos = "SELECT SUM(ValorPago) as ValorTotalPago, MAX(DataPagamento) as DataPagamento FROM PAGAMENTOS_COMISSOES " & _
                                            "WHERE ID_Venda = " & vendaId & " AND UserId = " & userIdGerencia & " AND TipoRecebedor = 'gerencia'"
                            Set rsPagamentos = connSales.Execute(sqlPagamentos)
                            ...
                            sqlPagamentos = "SELECT SUM(ValorPago) as ValorTotalPago, MAX(DataPagamento) as DataPagamento FROM PAGAMENTOS_COMISSOES " & _
                                            "WHERE ID_Venda = " & vendaId & " AND UserId = " & userIdCorretor & " AND TipoRecebedor = 'corretor'"
                            Set rsPagamentos = connSales.Execute(sqlPagamentos)
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <div><b><%= rsComissoes("Diretoria") %><br><%= rsComissoes("NomeDiretor") %></b></div>
                            ...
                            <% If Not IsEmpty(dataPagamentoDiretoria) Then %>
                                <br><small class="text-primary">Data Pag.: <%= dataPagamentoDiretoria %></small>
                            <% End If %>
                        ...
                        <td class="text-center">
                            <div><b><%= rsComissoes("Gerencia") %><br><%= rsComissoes("NomeGerente") %></b></div>
                            ...
                            <% If Not IsEmpty(dataPagamentoGerencia) Then %>
                                <br><small class="text-primary">Data Pag.: <%= dataPagamentoGerencia %></small>
                            <% End If %>
                        ...
                        <td class="text-center">
                            <div><b><%= rsComissoes("NomeCorretor") %></b></div>
                            ...
                            <% If Not IsEmpty(dataPagamentoCorretor) Then %>
                                <br><small class="text-primary">Data Pag.: <%= dataPagamentoCorretor %></small>
                            <% End If %>
    ...
    <!-- Novo Modal para Visualizar Pagamentos -->
    <div class="modal fade" id="viewPaymentsModal" tabindex="-1" aria-labelledby="viewPaymentsModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h6 class="mb-3" id="viewPaymentsRecipientName"></h6>
                    <div id="paymentsList">
                        <!-- Pagamentos serão carregados aqui via AJAX -->
                        <p class="text-center text-muted">Carregando pagamentos...</p>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    <script>
        ...
        // Função para excluir um pagamento específico
            function deletePayment(paymentId, idVenda, userId, recipientType, recipientName) {
                if (confirm(`Tem certeza que deseja excluir o pagamento ID ${paymentId}?`)) {
                    $.ajax({
                        ...
                        success: function(response) {
                            console.log('Objeto de resposta do servidor:', response);
                            const result = response;
                            if (result.success) {
                                alert('Pagamento excluído com sucesso!');
                                loadPayments(idVenda, userId, recipientType, recipientName);
                            } else {
                                alert('Erro ao excluir pagamento: ' + (result.message || 'Erro desconhecido.'));
                            }
                        },
                        error: function(xhr, status, error) {
                            alert('Erro na comunicação com o servidor ao tentar excluir o pagamento: ' + error);
                            console.error("Erro AJAX de exclusão:", status, error, xhr.responseText);
                        }
                    });
                }
            }
        ...
        $(document).ready(function() {
            $('#comissoesTable').DataTable({
                responsive: true,
                order: [[0, "desc"]],
                pageLength: 100, // Mostra 100 linhas por página
                ...
                initComplete: function() {
                    // Ajusta a largura das colunas após a inicialização
            ...
            function loadPayments(idVenda, userId, recipientType, recipientName) {
                const paymentsListDiv = $('#paymentsList');
                const recipientNameDisplay = $('#viewPaymentsRecipientName');
                paymentsListDiv.html('<p class="text-center text-muted">Carregando pagamentos...</p>');
                recipientNameDisplay.text(recipientName ? `Pagamentos para: ${recipientName}` : 'Todos os Pagamentos');

                let url = `get_pagamentos_por_comissao.asp?idVenda=${idVenda}`;
                if (userId) url += `&userId=${userId}`;
                if (recipientType) url += `&recipientType=${recipientType}`;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        paymentsListDiv.empty();
                        if (data.length > 0) {
                            let tableHtml = `
                                <table class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th class="text-center">ID Pagamento</th>
                            ...
                            data.forEach(function(payment) {
                                ...
                                tableHtml += `
                                    <tr>
                                        <td class="text-center">${payment.ID_Pagamento}</td>
                                        <td class="text-center">${new Date(payment.DataPagamento).toLocaleDateString('pt-BR')}</td>
                                        <td class="text-end">R$ ${formatCurrency(payment.ValorPago)}</td>
                                        <td class="text-center">${payment.Status}</td>
                                        <td class="text-center">${payment.UsuariosNome || 'N/A'} (${payment.TipoRecebedor})</td>
                                        <td>${payment.Obs || ''}</td>
                                        <td class="text-center">
                                            <button class="btn btn-danger btn-sm" onclick="deletePayment(${payment.ID_Pagamento}, ${idVenda}, '${currentUserId}', '${currentRecipientType}', '${recipientName}')">
                            ...
                            `;
                            paymentsListDiv.html(tableHtml);
                        } else {
                            paymentsListDiv.html('<p class="text-center text-muted">Nenhum pagamento encontrado para esta comissão.</p>');
                        }
                    },
                    error: function(xhr, status, error) {
                        paymentsListDiv.html('<p class="text-center text-danger">Erro ao carregar pagamentos: ' + error + '</p>');
            ...
            $('#viewPaymentsModal').on('show.bs.modal', function (event) {
                ...
                paymentsListDiv.html('<p class="text-center text-muted">Carregando pagamentos...</p>');
                recipientNameDisplay.text('');

                modal.data('diretoria', { id: button.data('diretoria-id'), nome: button.data('diretoria-nome') });
                modal.data('gerencia', { id: button.data('gerencia-id'), nome: button.data('gerencia-nome') });
                modal.data('corretor', { id: button.data('corretor-id'), nome: button.data('corretor-nome') });

                let viewRecipientSelect = $('#viewModalRecipient');
                if (viewRecipientSelect.length === 0) {
                    paymentsListDiv.before(`
                        <div class="mb-3">
                            <label for="viewModalRecipient" class="form-label">Filtrar por:</label>
                            <select class="form-select" id="viewModalRecipient" required>
                                <option value="">Todos os Pagamentos</option>
                            </select>
                        </div>
                    `);
                    viewRecipientSelect = $('#viewModalRecipient');
                }
                
                viewRecipientSelect.empty();
                viewRecipientSelect.append('<option value="">Todos os Pagamentos</option>');

                if (button.data('diretoria-id') && button.data('diretoria-nome') && button.data('diretoria-id') !== 0) {
                    viewRecipientSelect.append(`<option value="diretoria" data-user-id="${button.data('diretoria-id')}">${button.data('diretoria-nome')} (Diretoria)</option>`);
                }
                if (button.data('gerencia-id') && button.data('gerencia-nome') && button.data('gerencia-id') !== 0) {
                    viewRecipientSelect.append(`<option value="gerencia" data-user-id="${button.data('gerencia-id')}">${button.data('gerencia-nome')} (Gerência)</option>`);
                }
                if (button.data('corretor-id') && button.data('corretor-nome') && button.data('corretor-id') !== 0) {
                    viewRecipientSelect.append(`<option value="corretor" data-user-id="${button.data('corretor-id')}">${button.data('corretor-nome')} (Corretor)</option>`);
                }

                loadPayments(idVenda, null, null, "Todos os Pagamentos");

                viewRecipientSelect.off('change').on('change', function() {
                    const selectedOption = $(this).find(':selected');
                    const selectedType = selectedOption.val();
                    const selectedUserId = selectedOption.data('user-id');
                    const selectedName = selectedOption.text();
                    
                    loadPayments(idVenda, selectedUserId, selectedType, selectedName);
                });
            });
        });
    </script>
