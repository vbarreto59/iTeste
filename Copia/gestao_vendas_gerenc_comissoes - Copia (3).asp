                ...
                if (confirm(`Tem certeza que deseja excluir o pagamento ID ${paymentId}?`)) {
                    $.ajax({
                        ...
                        error: function(xhr, status, error) {
                            // Mensagem básica para o usuário
                            ...
                            try {
                                // Tenta parsear a resposta do servidor mesmo em caso de erro
                                const serverResponse = JSON.parse(xhr.responseText);
                                console.error("Resposta do servidor:", serverResponse);
                                
                                // Adiciona detalhes do servidor à mensagem do usuário se disponível
                                if (serverResponse.message) {
                                    errorMessage += '<div class="mt-2"><strong>Detalhes:</strong> ' + serverResponse.message + '</div>';
                                }
                                if (serverResponse.sql) {
                                    console.error("SQL executado:", serverResponse.sql);
                                }
                            } catch (e) {
                                console.error("Resposta bruta do servidor:", xhr.responseText);
                                errorMessage += '<div class="mt-2"><small>' + xhr.responseText + '</small></div>';
                            }
                            
                            errorMessage += '</div>';
                            paymentsListDiv.html(errorMessage);
...
// Modifique a função que processa o AJAX para tratar erros de JSON
function loadPayments(idVenda) {
    $('#paymentsTableBody').html('<tr><td colspan="7" class="text-center"><div class="spinner-border"></div></td></tr>');
                ...
                response.data.forEach(function(payment) {
                    html += `
                        <tr>
                            ...
                            <td>R$ ${payment.ValorPago.toFixed(2).replace('.', ',')}</td>
                            <td>${payment.UsuariosNome}</td>
                            <td>${payment.TipoRecebedor}</td>
            ...
            $('#paymentsTableBody').html(`
                <tr>
                    <td colspan="7" class="text-center text-danger">
                        Erro ao carregar pagamentos. Consulte o console para detalhes.
                    </td>
                </tr>
            `);
        }
...
// Função auxiliar para classes de status (mantida)
function getStatusBadgeClass(status) {
    ...
    return 'bg-secondary';
            ...
            // Função para carregar os pagamentos quando o modal é aberto
// Atualize o evento de abertura do modal
$('#viewPaymentsModal').on('show.bs.modal', function(event) {
    const button = $(event.relatedTarget);
    const idVenda = button.data('id-venda');
    
    // Limpa a tabela e mostra o spinner
    $('#paymentsTableBody').html('<tr><td colspan="7" class="text-center"><div class="spinner-border text-primary" role="status"></div></td></tr>');
    $('#noPaymentsMessage').hide();
    ...
    $.ajax({
                ...
                response.data.forEach(function(payment) {
                    html += `
                        <tr>
                            ...
                            <td>${payment.DataPagamento || 'N/A'}</td>
                            <td class="text-end">R$ ${(payment.ValorPago ? parseFloat(payment.ValorPago).toFixed(2).replace('.', ',') : '0,00')}</td>
                            <td>${payment.UsuariosNome || 'N/A'}</td>
                            <td>${payment.TipoRecebedor || 'N/A'}</td>
        ...
        error: function(xhr, status, error) {
            ...
            // Função auxiliar para determinar a classe CSS do badge de status
            function getStatusBadgeClass(status) {
                if (!status) return 'bg-secondary';
            ...
            // Funções auxiliares para formatação (simulando as funções ASP no JavaScript)
