...
<body>
                ...
                <table id="tabelaPessoas" class="table table-striped table-hover table-bordered" style="width:100%">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center-v th-media">Média do Cargo</th>
                    ...
                    <tbody>
                        ...
                        If dadosPessoas.Count > 0 Then
                            ...
                            ' ===============================================
                            ' CÁLCULO DAS MÉDIAS POR CARGO
                            ' ===============================================
                            Dim mediasCargo
                            Set mediasCargo = Server.CreateObject("Scripting.Dictionary")
                            
                            ' 1ª passada: acumula soma e contador por cargo
                            For Each pessoaKey In arrPessoas
                                Set infoPessoa = dadosPessoas(pessoaKey)
                                For Each cargoKey In infoPessoa("Cargos").Keys
                                    If Not mediasCargo.Exists(cargoKey) Then
                                        mediasCargo.Add cargoKey, Array(0, 0)   ' [soma dos valores, quantidade de pessoas]
                                    End If
                                    
                                    arrInfo = mediasCargo(cargoKey)
                                    arrInfo(0) = arrInfo(0) + infoPessoa("Cargos")(cargoKey)  ' soma valores
                                    arrInfo(1) = arrInfo(1) + 1                                ' conta pessoas
                                    mediasCargo(cargoKey) = arrInfo
                                Next
                            Next
                            
                            ' 2ª passada: calcular média de cada cargo
                            For Each cargoKey In mediasCargo.Keys
                                arrInfo = mediasCargo(cargoKey)
                                If arrInfo(1) > 0 Then
                                    Dim mediaCargoCalculada
                                    mediaCargoCalculada = arrInfo(0) / arrInfo(1)
                                    ' Substituir o array pela média calculada
                                    mediasCargo(cargoKey) = mediaCargoCalculada
                                Else
                                    mediasCargo(cargoKey) = 0
                                End If
                            Next
                            ...
                            For Each pessoaKey In arrPessoas
                                ...
                                For Each cargoEspecifico In cargosOrdenados
                                    If cargosPessoa.Exists(cargoEspecifico) Then
                                        ...
                                        Call ExibirLinhaCargo(pessoaKey, cargoEspecifico, cargosPessoa(cargoEspecifico), infoPessoa("TotalGeral"), primeiroCargo, linhaCargoCount, posicao, mediasCargo)
                                        primeiroCargo = False
                                    End If
                                Next
                                ...
                                For Each cargoKey In cargosPessoa.Keys
                                    ...
                                    If Not jaExibido Then
                                        linhaCargoCount = linhaCargoCount + 1
                                        Call ExibirLinhaCargo(pessoaKey, cargoKey, cargosPessoa(cargoKey), infoPessoa("TotalGeral"), primeiroCargo, linhaCargoCount, posicao, mediasCargo)
                                        primeiroCargo = False
                                    End If
                                Next
                                
                                ' Linha de total da pessoa
                                Dim percentualTotalPessoa, mediaPessoa
                                percentualTotalPessoa = (infoPessoa("TotalGeral") / totalGeralVTotal) * 100
                                
                                ' Calcular média dos cargos da pessoa
                                mediaPessoa = 0
                                If cargosPessoa.Count > 0 Then
                                    mediaPessoa = infoPessoa("TotalGeral") / cargosPessoa.Count
                                End If
                        ...
                        <tr class="total-pessoa <%= classePosicao %>">
                            ...
                            <td class="text-right-v">
                                <strong><%= FormatNumber(mediaPessoa, 2) %></strong>
                            </td>
                    ...
                    <tfoot>
                        <tr class="total-geral">
                            ...
                            <td class="text-right-v">
                                <strong>
                                <%
                                ' Calcular média geral dos cargos
                                Dim mediaGeralCargos, somaMediasCargos, countCargosDistintos
                                somaMediasCargos = 0
                                countCargosDistintos = 0
                                
                                For Each cargoKey In mediasCargo.Keys
                                    somaMediasCargos = somaMediasCargos + mediasCargo(cargoKey)
                                    countCargosDistintos = countCargosDistintos + 1
                                Next
                                
                                If countCargosDistintos > 0 Then
                                    mediaGeralCargos = somaMediasCargos / countCargosDistintos
                                    Response.Write FormatNumber(mediaGeralCargos, 2)
                                Else
                                    Response.Write "0,00"
                                End If
                                %>
    ...
    $(document).ready(function() {
        ...
        var table = $('#1tabelaPessoas').DataTable({
...
' ===============================================
Sub ExibirLinhaCargo(nomePessoa, cargo, valorCargo, totalPessoa, primeiroCargo, linhaCargoCount, posicaoPessoa, mediasCargo)
    ...
    ' Obter média do cargo
    If mediasCargo.Exists(cargo) Then
        mediaCargo = mediasCargo(cargo)
    Else
        mediaCargo = 0
    End If
