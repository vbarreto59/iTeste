...
' Função para converter número brasileiro para JavaScript - VERSÃO CORRIGIDA
Function ConverterParaJS(valor)
    Dim resultado
    
    
    
    ' Caso contrário, tenta limpar formatação brasileira
...
Dim ano, mes, trimestre, semestre, diretoriaID, whereClause, isFiltered
isFiltered = False
...
' --- NOVO: BUSCAR ANOS DISPONÍVEIS NO BANCO DE DADOS ---
...
<head>
    ...
    <style>
        ...
        /* Ajustar para garantir que todos os meses sejam visíveis em telas menores */
...
<body>
    <div class="container mt-4">
                ...
                <form method="post" class="row g-3">
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="ano" name="ano">
                            ...
                            <%
                            ' Popula o select apenas com anos que existem no banco
                            ...
                            ' Se não houver anos no banco, mostra mensagem
                            If anosCount = 0 Then
                    ...
                    <div class="col-12 d-flex gap-3 mt-2">
                        ...
                        <a href="gestao_vendas_diretoria1.asp" class="btn btn-secondary btn-filter">
        ...
        <div class="card mb-4 border-warning">
            ...
            <div class="card-body">
                <div class="row g-2">
                    ...
                    <% if ano <> "" then %>
                    <div class="col-auto">
                        <span class="badge bg-info p-2"><i class="fas fa-calendar me-1"></i> Ano: <%=ano%></span>
                    </div>
                    <% end if %>
                    <% if mes <> "" then %>
                    <div class="col-auto">
        ...
        <div class="resultados">
            ...
            If isFiltered Then
                ...
                totalVGV = 0
                If Not rs.EOF Then
                    totalVGV = ConverterParaJS(rs("TotalVGV"))
                End If
                rs.Close

                ' 3. Dados para o gráfico de VGV por mês
                ...
                podeMostrarGrafico = False
                
                ' SQL para o gráfico
                Dim whereClauseGrafico
                whereClauseGrafico = " WHERE Vendas.Excluido = 0"
                
                ' Aplicar filtro de Diretoria (se aplicável)
                If Not IsNull(diretoriaID) And Trim(CStr(diretoriaID)) <> "" And IsNumeric(diretoriaID) Then
                    whereClauseGrafico = whereClauseGrafico & " AND Vendas.DiretoriaId = " & CLng(diretoriaID)
                End If
                
                ' Aplicar filtro de Ano (obrigatório para gráfico)
                If ano <> "" And IsNumeric(ano) Then
                    whereClauseGrafico = whereClauseGrafico & " AND Vendas.AnoVenda = " & ano
                    podeMostrarGrafico = True
                End If
                    ...
                    Do While Not rs.EOF
                        If Not IsNull(rs("MesVenda")) Then
                            ...
                            If mesNum >= 1 And mesNum <= 12 Then
                                ...
                                mesesVGV(mesNum) = ConverterParaJS(rs("VGVMes"))
                                'Response.Write mesesVGV(mesNum)
                                'Response.end 
                            End If
                        End If
                        rs.MoveNext
                ...
                <!-- Título do ano exibido -->
                <div class="text-center mb-3">
                    <h5 class="text-muted">
                        VGV por mês em <strong><%=ano%></strong>
                        <% If mes <> "" And IsNumeric(mes) Then %>
                ...
                <!-- 12 CARDS DE VGV POR MÊS -->
                <div class="row mb-4 g-3">
                    <%
                    Dim valorMesFormatado, bgClass, isMesFiltrado
                    For i = 1 To 12
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        <div class="metric-card" style="background: #2ecc71;">
                            ...
                            <div class="metric-value">R$ <%=FormatNumber(totalVGV, 0)%></div>
                        </div>
                    </div>
                </div>
                
                <!-- GRÁFICO DE VGV POR MÊS -->
                <% If podeMostrarGrafico Then %>
                <div class="card mb-4">
                    <div class="card-header text-white" style="background: #9b59b6;">
                        <i class="fas fa-chart-line me-2"></i>VGV por Mês
                        <% if mes <> "" then %>
                        <span class="float-end badge bg-info">Mostrando dados anuais (filtro de mês não aplicado ao gráfico)</span>
                        <% end if %>
                    </div>
                    <div class="card-body">
                    ...
                    <div class="col-md-4">
                        <div class="card h-100">
                            <div class="card-header text-white" style="background: #f39c12;">
                                <i class="fas fa-user me-2"></i> Corretores
                            </div>
                            <div class="card-body">
                                <%
                                ...
                                If Not rs.EOF Then
                                    ...
                                    <div class="list-group list-group-flush">
                                        ...
                                        Do While Not rs.EOF
                                            cont = cont + 1
                                            vgvCorretor = ConverterParaJS(rs("VGV"))
                                            If totalVGV <> 0 Then
                                                percentual = (vgvCorretor / totalVGV) * 100
                                            Else
                                                percentual = 0
                                            End If
                                            %>
                                            <div class="list-group-item border-0 py-2 px-0">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div>
                                                        <strong><%=cont &"-"&rs("Corretor")%></strong><br>
                                                        <small class="text-muted"><%=rs("Unidades")%> unidades</small>
                                                    </div>
                                                    <div class="text-end">
                                                        <div class="fw-bold">R$ <%=FormatNumber(vgvCorretor, 0)%></div>
                                                        <small class="text-muted"><%=FormatNumber(percentual, 1)%>%</small>
                                                    </div>
                                                </div>
                                            </div>
    ...
    If isFiltered And podeMostrarGrafico Then
        ' Preparar dados JavaScript para o gráfico
        Response.Write "const mesesLabels = [" & _
                       """Jan"", ""Fev"", ""Mar"", ""Abr"", ""Mai"", ""Jun"", " & _
                       """Jul"", ""Ago"", ""Set"", ""Out"", ""Nov"", ""Dez""];" & vbCrLf
                       
        Response.Write "const vgvData = ["
        For i = 1 To 12
            Response.Write mesesVGV(i)
            If i < 12 Then Response.Write ", "
        Next
        Response.Write "];" & vbCrLf
        
        Response.Write "console.log('Dados do gráfico (convertidos):', vgvData);" & vbCrLf
    %>
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: function(context) {
                        ...
                        <% if mes <> "" And IsNumeric(mes) then %>
                        if (index + 1 == <%=CInt(mes)%>) {
                            return 'rgba(231, 76, 60, 0.7)'; // Vermelho para o mês filtrado
                        }
                        <% end if %>
                        return value > 0 ? 'rgba(52, 152, 219, 0.7)' : 'rgba(200, 200, 200, 0.3)';
                    },
                    borderColor: function(context) {
                        const index = context.dataIndex;
                        <% if mes <> "" And IsNumeric(mes) then %>
                        if (index + 1 == <%=CInt(mes)%>) {
                            return 'rgb(231, 76, 60)'; // Vermelho para o mês filtrado
                        }
                        <% end if %>
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                return 'VGV: R$ ' + value.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2});
                            },
